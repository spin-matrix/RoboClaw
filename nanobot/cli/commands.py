...
def gateway(
    ...
    gateway_server = GatewayServer(port=port, config=config)

    # Wire app channel into the gateway server
    app_channel = channels.get_channel("app")
    if app_channel is not None:
        gateway_server.app_channel = app_channel
